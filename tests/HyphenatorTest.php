...
it( 'matches locale shorthand exceptions for lowercase words', function (): void {
	$hyphenator = ps_decompose_word_make_hyphenator();

	expect( ps_decompose_word_soft_hyphens_visible( $hyphenator->hyphenate_text( 'digitaliseringsorganisasjon', 'nb_NO' ) ) )
...
it( 'matches global explicit exceptions across locales', function (): void {
	...
	expect( ps_decompose_word_soft_hyphens_visible( $hyphenator->hyphenate_text( 'Donaudampfschifffahrtsgesellschaft', 'de_DE' ) ) )
...
it( 'falls back to TeX pattern dictionaries when no exception exists', function ( string $word, string $locale ): void {
	$options = array_merge(
		ps_decompose_word_default_test_options(),
		array(
			'exceptions'      => '',
			'min_word_length' => 8,
		)
	);
	$hyphenator = ps_decompose_word_make_hyphenator( $options );

	expect( ps_decompose_word_soft_hyphens_visible( $hyphenator->hyphenate_text( $word, $locale ) ) )
		->toContain( '|' );
} )->with(
	array(
...
it( 'leaves words unchanged when no TeX dictionary is available for the locale', function (): void {
	...
	expect( ps_decompose_word_soft_hyphens_visible( $hyphenator->hyphenate_text( 'digitaliseringsorganisasjon', 'fi_FI' ) ) )
		->toBe( 'digitaliseringsorganisasjon' );
} );

it( 'hyphenates eligible HTML text nodes', function (): void {
	$processor = ps_decompose_word_make_html_processor();

	expect( ps_decompose_word_soft_hyphens_visible( $processor->process( '<h1>DIO Departementenes digitaliseringsorganisasjon</h1>', 'nb_NO' ) ) )
...
it( 'skips excluded HTML elements', function (): void {
	...
	expect( ps_decompose_word_soft_hyphens_visible( $processor->process( '<p>digitaliseringsorganisasjon <code>digitaliseringsorganisasjon</code></p>', 'nb_NO' ) ) )
